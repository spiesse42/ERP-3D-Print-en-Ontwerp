...
| `build_deploy.bat` | tests → frontend bouwen → kopie naar `addon/` → versie +1 → commit & push naar GitHub |
...
1. Maak op GitHub een **privé** repository aan, bv. `ERP-3D-Print-en-Ontwerp` (andere naam? pas dan ook `repository.json` en `addon/config.yaml` → `url` aan).
2. In deze map, eenmalig (opdrachtprompt):
   ...
   ```
3. `build_deploy.bat` draaien. De `.gitignore` houdt de databank, bijlagen, backups, `node_modules` en pdf's in de hoofdmap buiten GitHub; het script stopt als er toch een databank in `addon/` terechtkomt.
